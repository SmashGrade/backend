...
func Test_Gradetype_Find(t *testing.T) {
	...
	// Find Gradetype
	result2, err := repository.Find(db.GradeType1)
	gradetypes := result2.([]models.Gradetype)

	require.NoError(t, err)
	require.Nil(t, deep.Equal(db.GradeType1.ID, gradetypes[0].ID))
}
...
func Test_Gradetype_GetAll(t *testing.T) {
	...
	require.NoError(t, err)
	require.Nil(t, deep.Equal(db.GradeType1.ID, gradetypes[0].ID))
	require.Nil(t, deep.Equal(db.GradeType2.ID, gradetypes[1].ID))
}
...
func Test_Gradetype_GetID(t *testing.T) {
	...
	// Get by ID
	result, err := repository.GetId(db.GradeType1.ID)
	gradetype := result.(*models.Gradetype)

	require.NoError(t, err)
	require.Nil(t, deep.Equal(gradetype.Description, db.GradeType1.Description))
}
...
func Test_Gradetype_DeleteId(t *testing.T) {
	...
	// Delete gradetype
	err := repository.DeleteId(db.GradeType1.ID)
